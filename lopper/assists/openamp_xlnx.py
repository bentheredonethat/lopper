...
def xlnx_openamp_normalize_baremetal_timers(sdt, target_os, machine):
    """Restore native bindings for timers selected by a bare-metal domain.
    ...
    Algorithm:
        Linux-compatible UIO properties come from the domain YAML and Zephyr
        does not use the Linux ``cdns,ttc`` binding.  Only bare-metal output
        needs the relation-selected timer restored after a global Linux UIO
    ...
    timer_phandles = []
    for relation in relation_groups[0].subnodes(children_only=True):
    ...
    # The shared source tree may contain Linux's UIO override.  Bare-metal
    ...
    for timer_phandle in timer_phandles:
        ...
        is_linux_uio = (timer_node is not None and "uio" in timer_node.propval("compatible"))
        if is_linux_uio:
...
def xlnx_openamp_parse(sdt, options, verbose = 0 ):
    ...
    xlnx_openamp_normalize_baremetal_timers(
